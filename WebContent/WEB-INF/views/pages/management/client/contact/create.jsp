			...
			<div class="box-body form-horizontal">
				...
				<c:if test="${!empty clientes}">
					<div class="form-group">
						<label class="control-label col-sm-2" for="idCliente">{{'CLIENT'
							| translate}}</label>
						<div class="col-sm-10">
						 	<form:select class="form-control" path="idCliente">							
								<form:option value="-1">-- {{"SELECT" | translate}} --</form:option>
								<form:options items="${clientes}" itemValue="id" itemLabel="nomeFantasia" />
							</form:select>
						</div>
					</div>
				</c:if>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						<form:errors path="idSequencia" cssStyle="error" />
					</div>
				</div>
				<div class="form-group">
					<label class="control-label col-sm-2" for="tipoContato">{{'CLIENT_CONTACT_TYPE'
						| translate}}:</label>
					<div class="col-sm-10">
					 	<form:select class="form-control" path="tipoContato">							
							<form:option value="-1">-- {{"SELECT" | translate}} --</form:option>
							<form:options items="${tiposContato}" itemValue="tipo" itemLabel="descricao" />
						</form:select>
						<form:errors path="tipoContato" cssClass="error" />
					</div>
				</div>
				<div class="form-group">
